...
class Interceptor extends \Magento\Theme\Model\ResourceModel\Theme\File\Collection implements \Magento\Framework\Interception\InterceptorInterface
{
    ...
    public function setDefaultOrder($direction = 'ASC')
    {
        ...
        if (!$pluginInfo) {
            return parent::setDefaultOrder($direction);
        } else {
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'addThemeFilter');
        if (!$pluginInfo) {
            return parent::addThemeFilter($theme);
        } else {
            return $this->___callPlugins('addThemeFilter', func_get_args(), $pluginInfo);
        }
    }
