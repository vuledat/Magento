...
class Interceptor extends \Magento\Customer\Model\ResourceModel\Form\Attribute\Collection implements \Magento\Framework\Interception\InterceptorInterface
{
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'addFormCodeFilter');
        if (!$pluginInfo) {
            return parent::addFormCodeFilter($code);
        } else {
            return $this->___callPlugins('addFormCodeFilter', func_get_args(), $pluginInfo);
        }
    }
    ...
    public function setSortOrder($direction = 'ASC')
    {
        ...
        if (!$pluginInfo) {
            return parent::setSortOrder($direction);
        } else {
