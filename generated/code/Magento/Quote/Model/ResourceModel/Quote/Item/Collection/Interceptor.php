...
class Interceptor extends \Magento\Quote\Model\ResourceModel\Quote\Item\Collection implements \Magento\Framework\Interception\InterceptorInterface
{
    ...
    {
        $this->___init();
        parent::__construct($entityFactory, $logger, $fetchStrategy, $eventManager, $entitySnapshot, $itemOptionCollectionFactory, $productCollectionFactory, $quoteConfig, $connection, $resource, $storeManager);
    }
    ...
    {
        $pluginInfo = $this->pluginList->getNext($this->subjectType, 'resetJoinQuotes');
        if (!$pluginInfo) {
            return parent::resetJoinQuotes($quotesTableName, $productId);
        } else {
            return $this->___callPlugins('resetJoinQuotes', func_get_args(), $pluginInfo);
        }
    }
